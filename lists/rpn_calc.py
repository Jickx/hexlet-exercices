...
# так: 1 2 + 4 * 3 +, а результат вычисления: 15. Другой пример - выражение:
...
def rpn_calc(items: list) -> int:
    ...
    for item in items:
        if isinstance(item, int):
            ...
            second = numbers.pop()
            first = numbers.pop()
            numbers.append(operators[f'{item}'](first, second))
    return numbers[0]


print(rpn_calc([1, 2, '+', 4, '*', 3, '+']))  # 15
print(rpn_calc([7, 2, 3, '*', '-']))  # 1
print(rpn_calc([1, 2, '+', 4, '*', 3, '/']))  # 4
